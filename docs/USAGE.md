...
同学：把 [README](../README.md) 或 [AGENT_PROMPT.md](AGENT_PROMPT.md) 里的提示词**整段**发给编码 Agent，不要只说「帮我装一下」。人准备 Python 3.11+、至少一种翻译 Key（腾讯/百度/阿里/百炼/DeepSeek 均可）。不要做成安装包给人下载。不要拷贝**别人的** `.env` / `.venv` / `data/`。把 Key 发给 Agent 或让它写入本地 `.env` 时，Agent 应该帮写。
...
按顺序做。麦克风：启动后点系统弹窗「允许」，不要先让使用者去系统设置里找。只有曾经点过「不允许」时，才用应用里的按钮打开麦克风设置。Key：使用者没给、也没让你写时停下来；他发来 Key 或明确要求写入本地 `.env` 时写入对应行，不要朗读、不要 commit。
...
应用自己向系统申请。启动后（或第一次点「新建一节课」）会弹出系统对话框，点「允许」即可。**不要先去系统设置里翻开关。**

若以前点过「不允许」，应用会提供「打开麦克风设置」按钮，直接跳到麦克风开关页；打开后**完全退出应用再打开**（不要只关窗口）。
