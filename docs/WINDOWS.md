...
仓库里的 **main 仍是 macOS 上课版**。Windows 上请开 `feat/windows` 再改代码。这份文档按第一次定的适配逻辑写：业务层基本不用动，只换「系统壳」。
...
可另做 `run.bat`：`.\.venv\Scripts\python.exe -u main.py`。每人自己填 `.env`，不要拷贝别人的 Key。
