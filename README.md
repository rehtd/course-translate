...
3. 在 `.env` 至少填一种翻译 Key（腾讯云 / 百度 / 阿里 / 百炼 / DeepSeek 均可；不要用别人的）。课后笔记才需要 DeepSeek。
...
英文识别是本地 Whisper。上课不必用 DeepSeek；课后笔记整理才走 DeepSeek。详情见 [docs/USAGE.md](docs/USAGE.md#翻译引擎怎么选)。

| 引擎 | 优点 | 缺点 |
|------|------|------|
| DeepSeek | 课堂中文最顺；吃术语表和上下文；课后笔记也用它 | 要联网、按量计费；上课不是必须 |
| 阿里百炼 Qwen | 也吃术语表；常有免费额度 | 另申请 Key；质量通常不如 DeepSeek 稳 |
| 百度 / 阿里 / 腾讯云机器翻译 | 快、有免费额度（腾讯每月约 500 万字符） | 不吃术语表；课名/人名易乱译 |
